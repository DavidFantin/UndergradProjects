...
public class PriorityScheduler extends Scheduler {
	...
	protected class PriorityQueue extends ThreadQueue {
		...
		protected ThreadState pickNextThread() {
			...
			ThreadState highestPriorityTS = TSQ.getFirst();
			for (ThreadState state : TSQ) {
				if (highestPriorityTS.getEffectivePriority() < state.getEffectivePriority()) {
					highestPriorityTS = state;
				}
			}
			TSQ.remove(highestPriorityTS);
			return highestPriorityTS;
		}

		public void print() {
			Lib.assertTrue(Machine.interrupt().disabled());
			// implement me (if you want) -> How 'bout not?
		}
		...
		public LinkedList<ThreadState> TSQ = new LinkedList<ThreadState>();
	}
	...
	protected class ThreadState {
		...
		public int getEffectivePriority() {
			...
			for (PriorityQueue PQ : PQ_Owned) {
				if (PQ.transferPriority) {
					for (ThreadState st : PQ.TSQ) {
		...
		/** A linked list that contains all the priority queues owned by the associated thread. */
		protected LinkedList<PriorityQueue> PQ_Owned = new LinkedList<PriorityQueue>();
	}
}